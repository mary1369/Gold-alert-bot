...
function normalize(a) { return a.map(c => ({ time: num(c.time ?? c.t) * (num(c.time ?? c.t) < 1e10 ? 1000 : 1), open: num(c.open ?? c.o), high: num(c.high ?? c.h), low: num(c.low ?? c.l), close: num(c.close ?? c.c), volume: num(c.volume ?? c.v) ?? 0 })).filter(c => c.time && c.open != null && c.high != null && c.low != null && c.close != null).sort((a,b) => a.time-b.time); }
...
function mss(d,s,dir){const look=d.slice(-10); if(look.length<5)return false; const mid=look.slice(0,-2); const hi=Math.max(...mid.map(c=>c.high)), lo=Math.min(...mid.map(c=>c.low)); const last=d.at(-1); return dir==='BUY'?last.close>hi:last.close<lo;}
...
function fib(d,dir){const s=structure(d);if(!s)return null;const hi=s.h2.p,lo=s.l2.p,range=hi-lo;if(range<=0)return null;const xs=[.236,.382,.5,.618,.705,.786,1,1.272,1.618],levels={};for(const x of xs)levels[x]=dir==='BUY'?hi-range*x:lo+range*x;levels[1.272]=dir==='BUY'?lo+range*1.272:hi-range*1.272;levels[1.618]=dir==='BUY'?lo+range*1.618:hi-range*1.618;return{levels,range};}
function fibNear(d,dir){const f=fib(d,dir);if(!f)return{ok:false};const p=d.at(-1).close,tol=Math.max(f.range*.05,0.5);let near=null,best=Infinity;for(const x of [.236,.382,.618,.705,.786]){const dist=Math.abs(p-f.levels[x]);if(dist<best){best=dist;near=x;}}return{ok:best<=tol,near,levels:f.levels,range:f.range};}
...
function analyze(d){if(d.length<200)return null;const m5=d.slice(0,-1),m15=aggregate(m5,15),h1=aggregate(m5,60),h4=aggregate(m5,240);if(h4.length<20||h1.length<50||m15.length<50)return null;const h4b=bias(h4),h1b=bias(h1),m15b=bias(m15),s=structure(m5),a=atr(m5);if(!s||!a)return null;let dir=null;if(h4b==='BULLISH'&&h1b==='BULLISH'&&m15b!=='BEARISH'&&s.trend==='BULLISH')dir='BUY';if(h4b==='BEARISH'&&h1b==='BEARISH'&&m15b!=='BULLISH'&&s.trend==='BEARISH')dir='SELL';if(!dir)return null;const c=m5.at(-1),p=m5.at(-2),b=bos(m5,s,dir),sw=sweep(c,s,dir),ms=mss(m5,s,dir),ob=zone(m5,dir),fv=fvg(m5,dir),z=fv||ob,ret=inZone(c,z),disp=displacement(c,a,dir),fibc=fibNear(m5,dir),of=loadOF();let score=0;if(h4b===dirToBias(dir))score+=2;if(h1b===dirToBias(dir))score+=2;if(m15b===dirToBias(dir))score+=1;if(sw)score+=2;if(ms)score+=2;if(b)score+=1;if(ob)score+=1;if(fv)score+=1;if(ret)score+=1;if(disp)score+=2;if(fibc.ok)score+=1;if(of.available&&ofConfirm(of,dir))score+=2;if(score<MIN_SCORE||!sw||!ms||!disp||!z||!ret)return null;const sl=dir==='BUY'?Math.min(z.low,s.l2.p)-a*.15:Math.max(z.high,s.h2.p)+a*.15,entry=c.close,risk=Math.abs(entry-sl);if(risk<a*.45||risk>a*2.2)return null;const fl=fibc.levels||{};return{direction:dir,entry,sl,tp1:entry+(dir==='BUY'?risk:-risk),tp2:entry+(dir==='BUY'?risk*2:-risk*2),tp3:fl[1.618]??entry+(dir==='BUY'?risk*3:-risk*3),score,h4:h4b,h1:h1b,m15:m15b,m5:s.trend,atr:a,sweep:sw,mss:ms,bos:b,ob:Boolean(ob),fvg:Boolean(fv),fibNear:fibc.near,fibLevels:fl,orderflow:of,displacement:disp,candleTime:c.time};}
...
function msg(s){const f=s.fibLevels||{},o=s.orderflow;return`${s.direction==='BUY'?'🟢':'🔴'} XAUUSD ${s.direction} V3\n\n⏱ M5 closed: ${fmt(s.candleTime)}\n💰 Entry: ${r(s.entry)}\n🛑 SL: ${r(s.sl)}\n🎯 TP1: ${r(s.tp1)}\n🎯 TP2: ${r(s.tp2)}\n🎯 TP3 / Fib 1.618: ${r(s.tp3)}\n\n🏛 H4: ${s.h4}\n📊 H1: ${s.h1}\n📈 M15: ${s.m15}\n🧭 M5: ${s.m5}\n\n💧 Sweep: ${s.sweep?'✅':'❌'}\n🧠 MSS: ${s.mss?'✅':'❌'} | BOS: ${s.bos?'✅':'❌'}\n📦 OB: ${s.ob?'✅':'❌'} | FVG: ${s.fvg?'✅':'❌'}\n🎯 Fib near: ${s.fibNear??'-'}\nFib 0.236 ${r(f[.236])} | 0.382 ${r(f[.382])}\nFib 0.618 ${r(f[.618])} | 0.705 ${r(f[.705])} | 0.786 ${r(f[.786])}\nFib 1.272 ${r(f[1.272])} | 1.618 ${r(f[1.618])}\n\n🧬 Order Flow: ${o.available?'CONNECTED':'WAITING'}\nΔ ${r(o.delta)} | CVD slope ${r(o.cvdSlope)}\nImbalance: ${o.imbalance||'-'} | Absorption: ${o.absorption||'-'}\n⚡ Displacement: ${s.displacement?'✅':'❌'}\n⭐ Score: ${s.score}\n📏 ATR: ${r(s.atr)}\n\n⚠️ Risk management mandatory.`;}
