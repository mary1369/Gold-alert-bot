...
function signalAt(idx){
  ...
  const t4=trend(h4),t1=trend(h1),t15=trend(m15),c=a.at(-1),atrv=atr(a); if(!atrv)return null;
  const prev6=a.slice(-7,-1), priorHi=Math.max(...prev6.map(x=>x.high)), priorLo=Math.min(...prev6.map(x=>x.low));
  const range=Math.max(c.high-c.low,1e-9),body=Math.abs(c.close-c.open);
  const displacement=body/range>=0.5&&range>=atrv*0.65;
  ...
  // Support/resistance reversal path: detects a clean reaction from a nearby level,
  // then asks for a directional candle and a small MSS-style confirmation.
  const zone=a.slice(-21,-1);
  const support=Math.min(...zone.map(x=>x.low)),resistance=Math.max(...zone.map(x=>x.high));
  const nearSupport=Math.abs(c.low-support)<=atrv*0.45 || (c.low<=support+atrv*0.25&&c.close>support);
  const nearResistance=Math.abs(c.high-resistance)<=atrv*0.45 || (c.high>=resistance-atrv*0.25&&c.close<resistance);
  const bullishRejection=nearSupport&&c.close>c.open&&c.close>support;
  const bearishRejection=nearResistance&&c.close<c.open&&c.close<resistance;
  const prev3=a.slice(-4,-1);
  const localMssBuy=c.close>Math.max(...prev3.map(x=>x.high));
  const localMssSell=c.close<Math.min(...prev3.map(x=>x.low));
  const trendBull=t4==='BULLISH'&&t1==='BULLISH';
  const trendBear=t4==='BEARISH'&&t1==='BEARISH';

  let dir=null,score=0,setup='';
  if(trendBull&&t15!=='BEARISH'&&(bullishBreak||sweepBuy)){
    dir='BUY'; setup='TREND'; score=4+(bullishBreak?2:0)+(sweepBuy?1:0)+(displacement?1:0);
  } else if(trendBear&&t15!=='BULLISH'&&(bearishBreak||sweepSell)){
    dir='SELL'; setup='TREND'; score=4+(bearishBreak?2:0)+(sweepSell?1:0)+(displacement?1:0);
  } else if(bullishRejection&&(localMssBuy||displacement)){
    dir='BUY'; setup='SUPPORT_REJECTION'; score=3+2+(localMssBuy?2:0)+(displacement?1:0)+(trendBull?1:0);
  } else if(bearishRejection&&(localMssSell||displacement)){
    dir='SELL'; setup='RESISTANCE_REJECTION'; score=3+2+(localMssSell?2:0)+(displacement?1:0)+(trendBear?1:0);
  }
  if(!dir||score<MIN_SCORE)return null;

  const entry=c.close,swingLow=Math.min(...a.slice(-12).map(x=>x.low)),swingHigh=Math.max(...a.slice(-12).map(x=>x.high));
  ...
  return {direction:dir,setup,entry,sl,tp1:dir==='BUY'?entry+risk:entry-risk,tp2:dir==='BUY'?entry+risk*2:entry-risk*2,score,candleTime:c.time,h4:t4,h1:t1,m15:t15,m5:bullishBreak?'BULLISH':bearishBreak?'BEARISH':'RANGE',breakout:dir==='BUY'?bullishBreak:bearishBreak,sweep:dir==='BUY'?sweepBuy:sweepSell,displacement,localMss:dir==='BUY'?localMssBuy:localMssSell,support,resistance,atr:atrv};
}
...
if(!found){console.log('NO SIGNAL: no qualifying trend, support-rejection, or resistance-rejection setup in recent closed candles.');process.exit(0);}
console.log('SIGNAL CANDIDATE',JSON.stringify(found));
const state=load(STATE,{}),key=`${found.direction}|${found.setup}|${found.candleTime}|${found.entry.toFixed(2)}|${found.sl.toFixed(2)}`;
