...
function simulate(d, engine, start=300) {
  ...
  return {signals,trades:trades.length,wins:wins.length,losses:losses.length,winRate:+(trades.length?wins.length/trades.length:0).toFixed(4),tp1HitCount:trades.filter(x=>x.tp1Hit).length,netR:+netR.toFixed(3),profitFactor:+(grossWin/(grossLoss||1)).toFixed(3),maxDrawdownR:+maxDD.toFixed(3),avgR:+(netR/(trades.length||1)).toFixed(3),details:trades};
}
...
(async()=>{try{
  ...
  const out={source:'Dukascopy XAUUSD spot M5',days,candles:d.length,periodStart:d[0].time,periodEnd:d.at(-1).time,logicSource:'server_v6.js production analyze() loaded directly; no reconstructed strategy',execution:'closed M5 signal; next M5 open entry; actual V6 SL/TP2; TP1-hit path recorded; one position at a time',result};
