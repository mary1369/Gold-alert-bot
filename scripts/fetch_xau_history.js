...
async function biquoteM5Window(from,to){
 const p=new URLSearchParams({interval:'5m',limit:'1000',from:new Date(from).toISOString(),to:new Date(to).toISOString()});
 const u=`https://biquote.io/api/XAUUSD/ohlc?${p}`;
 ...
 if(!Array.isArray(rows)||!rows.length)throw new Error('BiQuote returned no XAUUSD M5 bars');
...
async function biquoteM5(){
 const end=NOW-M5;
 const windows=[
   [end-14*DAY,end-7*DAY],
   [end-7*DAY,end]
 ];
 const all=[];
 for(const [from,to] of windows){const got=await biquoteM5Window(from,to);console.log(`BiQuote M5 window ${new Date(from).toISOString()} -> ${new Date(to).toISOString()}: ${got.length}`);all.push(...got)}
 return unique(all);
}
...
(async()=>{
 let bars=[];
 const providers=[
   ['BiQuote M5 14d/2 windows',()=>biquoteM5()],
 ...
 for(const [name,fn] of providers){
   try{const got=unique(await fn());console.log(`${name}: ${got.length}`);bars=unique([...bars,...got]);if(fresh(bars))break}
   catch(e){console.log(`${name} failed: ${e?.stack||e?.message||String(e)}`)}
 }
